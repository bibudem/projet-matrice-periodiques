...
module.exports = class ListeStatistique {
  ...
  static async rapportStatistiquePlateforme(annees,plateforme) {
    ...
    return  db.execute('SELECT titre,ISSN,EISSN,statut,abonnement,bdd,domaine,fournisseur,plateforme,tbl_periodiques.idRevue as idP,  Total_Item_Requests,  Unique_Item_Requests,  No_License,  citations , articlesUdem,  JR5COURANT,  JR5INTER,  JR5RETRO, tbl_statistiques.dateA as dateAS, tbl_statistiques.dateM as dateMS FROM `tbl_periodiques` LEFT JOIN tbl_statistiques ON tbl_periodiques.idRevue=tbl_statistiques.idRevue where ('+sqlAnnees+') ' +sqlPlateforme+ ' order by tbl_periodiques.idRevue ',annees);
