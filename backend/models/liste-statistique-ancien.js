...
module.exports = class ListeStatistique {
  ...
  static async rapportStatistiquePeriode(annees,plateforme) {
    ...
    /*let sql = "SELECT titre,ISSN,EISSN,statut,abonnement,bdd,domaine,tbl_periodiques.idRevue as idP, sum(Total_Item_Requests) as Total_Item_Requests, SUM(Unique_Item_Requests) AS Unique_Item_Requests, SUM(No_License) AS No_License, SUM(citations) AS citations , SUM(articlesUdem) AS articlesUdem, sum(JR4COURANT) as JR4COURANT, sum(JR4INTER) as JR4INTER, sum(JR4RETRO) as JR4RETRO,(SELECT GROUP_CONCAT(plateforme, ' ') from `tbl_statistiques` WHERE `idRevue` = idP and "+ sqlAnnees +" ) as plateforme FROM `tbl_periodiques` LEFT JOIN tbl_statistiques ON tbl_periodiques.idRevue=tbl_statistiques.idRevue where "+ sqlAnnees +" "+ sqlPlateforme +" GROUP BY tbl_periodiques.idRevue order by tbl_periodiques.idRevue ";
    console.log('sql: ', SqlString.format(sql,cloneAnnees))*/
    return  db.execute("SELECT titre,ISSN,EISSN,statut,abonnement,bdd,domaine,fournisseur,tbl_periodiques.idRevue as idP, sum(Total_Item_Requests) as Total_Item_Requests, SUM(Unique_Item_Requests) AS Unique_Item_Requests, SUM(No_License) AS No_License, SUM(citations) AS citations , SUM(articlesUdem) AS articlesUdem, sum(JR4COURANT) as JR4COURANT, sum(JR4INTER) as JR4INTER, sum(JR4RETRO) as JR4RETRO, sum(JR3OAGOLD) as JR3OAGOLD,(SELECT GROUP_CONCAT(plateforme, ' ') from `tbl_statistiques` WHERE `idRevue` = idP and "+ sqlAnnees +"  ) as plateforme FROM `tbl_periodiques` LEFT JOIN tbl_statistiques ON tbl_periodiques.idRevue=tbl_statistiques.idRevue where "+ sqlAnnees +" "+ sqlPlateforme +" GROUP BY tbl_periodiques.idRevue order by tbl_periodiques.idRevue ",cloneAnnees);
  }
  ...
  static async rapportStatistiquePlateforme(annees,plateforme) {
    ...
    return  db.execute('SELECT titre,ISSN,EISSN,statut,abonnement,bdd,domaine,fournisseur,plateforme,tbl_periodiques.idRevue as idP,  Total_Item_Requests,  Unique_Item_Requests,  No_License,  citations , articlesUdem,  JR4COURANT,  JR4INTER,  JR4RETRO, tbl_statistiques.dateA as dateAS, tbl_statistiques.dateM as dateMS FROM `tbl_periodiques` LEFT JOIN tbl_statistiques ON tbl_periodiques.idRevue=tbl_statistiques.idRevue where ('+sqlAnnees+') ' +sqlPlateforme+ ' order by tbl_periodiques.idRevue ',annees);
